...
async def exec_command(workspace_id: str, body: ExecCommandRequest) -> dict:
...
async def list_files(workspace_id: str, path: str = ".") -> dict:
...
async def write_file(workspace_id: str, body: WriteFileRequest) -> dict:
    from app.security.validator import workflow_validator
...
async def edit_file(workspace_id: str, body: EditFileRequest) -> dict:
...
async def delete_file(workspace_id: str, path: str) -> dict:
    await workspace_manager.delete_file(workspace_id, path)
    return {"status": "deleted", "path": path}


@router.get("/{workspace_id}/history")
async def execution_history(workspace_id: str) -> dict:
...
async def clone_repo(workspace_id: str, repo_url: str, branch: str = "main") -> dict:
...
async def commit_push(workspace_id: str, message: str, branch: str | None = None) -> dict:
